...
BEGIN_ENUM(RestFlags, uint16_t)
	EV(CanShortRest, 1)
	EV(CanLongRest, 2)
	EV(Unknown0x04, 4)
END_ENUM()

BEGIN_ENUM(RestErrorFlags, uint8_t)
	EV(GlobalDisabled, 1)
	EV(Script, 2)
	EV(Dialog, 4)
	EV(Combat, 8)
...
BEGIN_ENUM(TravelFlags, uint16_t)
	EV(BlockFastTravel, 1)
	EV(BlockCampTravel, 2)
END_ENUM()

BEGIN_ENUM(TravelErrorFlags, uint16_t)
	EV(InDanger, 2)
	EV(Condition, 4)
	EV(DangerZone, 8)
	EV(Dialog, 0x10)
...
BEGIN_ENUM(FleeErrorFlags, uint8_t)
	EV(Region, 1)
	EV(EnemyTooClose, 2)
	EV(CantMove, 4)
	EV(Downed, 8)
