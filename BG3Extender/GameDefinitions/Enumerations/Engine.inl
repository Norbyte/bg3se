...
BEGIN_ENUM(AnimationInstanceChangeType, uint32_t)
    EV(StartUpdate, 0)
    EV(StopUpdate, 1)
END_ENUM()
...
BEGIN_BITMASK(AppliedMaterialDirtyFlags, uint8_t)
    EV(BlendStateID, 1)
...
BEGIN_BITMASK(OverlayMaterialRequestFlags, uint8_t)
    EV(Overlay, 1)
    EV(Fading, 2)
    EV(Remove, 4)
...
BEGIN_BITMASK(ApplyMaterialMapFlags, uint32_t)
    EV(BaseMap, 1)
    EV(NormalMap, 2)
    EV(PhysicalMap, 4)
    EV(GlowMap, 8)
...
BEGIN_BITMASK(OverlayMaterialApplyFlags, uint8_t)
    EV(Body, 1)
    EV(Armor, 2)
    EV(Weapon, 4)
    EV(Wings, 8)
...
BEGIN_BITMASK(MaterialFlags, uint32_t)
...
BEGIN_BITMASK(MaterialUsedWithFlags, uint32_t)
    EV(FixedRibbons, 0x01)
    EV(CameraAlignedRibbons, 0x02)
END_ENUM()

BEGIN_BITMASK(MaterialShaderDescFlags, uint16_t)
    EV(Exposure, 0x01)
    EV(AO, 0x02)
    EV(ShadowMask, 0x04)
...
BEGIN_BITMASK(RenderPass, uint32_t)
    EV(Opaque, 0x01)
    EV(Unknown2, 0x02)
    EV(EarlyAlpha, 0x100)
...
BEGIN_BITMASK(VisualRequestFlags, uint8_t)
...
BEGIN_BITMASK(LightFlags, uint8_t)
...
BEGIN_BITMASK(ProvidedMultiEffectObjectFlags, uint32_t)
    EV(HasEntity, 0x1)
    EV(HasPosition, 0x2)
END_ENUM()
