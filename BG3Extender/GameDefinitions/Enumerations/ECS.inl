BEGIN_ENUM(ExtSystemType, uint32_t)
    E(PickingHelper)
    E(ServerDialog)
    E(AnimationBlueprint)
    E(AnimationSet)
    E(Effect)
    E(VisualChange)
    E(VisualChanged)
    E(Visual)
    E(Light)
    E(FadeableObstructionMapping)

    E(LevelInstanceAttachRequest)
    E(LevelInstanceLoadRequest)
    E(LevelInstanceMove)
    E(LevelInstanceAttach)
    E(LevelInstanceLoad)
    E(InstancingRequest)

    E(ServerShapeshift)
    E(ServerCastRequest)
    E(ServerSpell)
    E(ServerCombat)
    E(ServerTurnOrder)
    E(ServerActionResource)
    E(ServerBoost)
    E(ServerPassive)
    E(ServerRoll)
    E(ServerStats)
    E(ServerVisual)
    E(ServerSpellCooldown)
    E(ServerSpellLearning)
    E(ServerHit)
    E(ServerProgression)
    E(ServerInterruptDecision)
    E(ServerInterruptRequests)
    E(ServerInterruptManagement)
    E(ServerSummonSpawn)
    E(ServerSummonDespawn)
    E(ServerCapabilities)
    E(ServerTemplateChange)
    E(ServerConcentration)
    E(ServerExperience)
    E(ServerShortRest)
    E(ServerLongRest)
    E(ServerFTBZone)
    E(ServerStatusRequest)
    E(ServerInventoryCanPlace)
    E(ServerInventoryReceivalNotification)
    E(ServerInventoryEquipment)
    E(ServerInventoryInteractionRequest)
    E(ServerInventoryInteraction)
    E(ServerInventoryLocking)
    E(ServerMagicPocketsTracking)
    E(ServerInventoryManagement)
    E(ServerNewInventoryMember)
    E(ServerInventoryStack)
    E(ServerTradeBuyback)
    E(ServerTreasureGeneration)
    E(ServerGod)
    E(ServerRestore)
    E(ServerBodyType)
    E(ServerDualWielding)
    E(ServerFalling)
    E(ServerDisplayName)
    E(ServerGravity)
    E(ServerLeader)
    E(ServerPartyTeleport)
    E(ServerPingRequest)
    E(ServerPlatform)
    E(ServerRating)
    E(ServerAttitude)
    E(ServerParty)
    E(ServerAi)
    E(ServerSightViewshed)
    E(ServerRollSave)
    E(ServerCombatLog)
    E(ServerLockpicking)
    E(ServerHotbar)

    E(ClientEquipmentVisuals)
    E(ClientVisual)
    E(ClientCharacterIconRender)
    E(ClientCharacterManager)
    E(ClientCamera)
    E(ClientVisualsVisibilityState)
    E(ClientEffectHandler)
    E(ClientTimeline)
    E(ClientConstruction)

    E(SoundRouting)
    E(Max)
END_ENUM()

BEGIN_ENUM(ExtResourceManagerType, uint32_t)
    E(AbilityDistributionPreset)
    E(AbilityList)
    E(ActionResourceGroup)
    E(ActionResource)
    E(ApprovalRating)
    E(AreaLevelOverride)
    E(AvatarContainerTemplate)
    E(BackgroundGoal)
    E(Background)
    E(CalendarDayRange)
    E(CampChestTemplate)

    E(AbilityDefaultValues)
    E(EquipmentDefaultValues)
    E(FeatDefaultValues)
    E(PassiveDefaultValues)
    E(PreparedSpellDefaultValues)
    E(SkillDefaultValues)
    E(SpellDefaultValues)

    E(CharacterCreationAccessorySet)
    E(CharacterCreationAppearanceMaterial)
    E(CharacterCreationAppearanceVisual)
    E(CharacterCreationEquipmentIcons)
    E(CharacterCreationEyeColor)
    E(CharacterCreationIconSettings)
    E(CharacterCreationHairColor)
    E(CharacterCreationMaterialOverride)
    E(CharacterCreationPassiveAppearance)
    E(CharacterCreationPreset)
    E(CharacterCreationSharedVisual)
    E(CharacterCreationSkinColor)
    E(CharacterCreationVOLine)

    E(CinematicArenaFrequencyGroup)
    E(ClassDescription)
    E(ColorDefinition)
    E(CompanionPreset)
    E(CustomDice)
    E(ConditionError)
    E(DeathTypeEffect)
    E(DifficultyClass)
    E(DisturbanceProperty)
    E(DLC)
    E(EncumbranceType)
    E(EquipmentList)
    E(EquipmentType)
    E(ExperienceReward)
    E(Faction)
    E(FeatSoundState)
    E(Feat)
    E(FeatDescription)
    E(FlagSoundState)
    E(God)
    E(GoldReward)
    E(Gossip)
    E(FixedHotBarSlot)
    E(ItemThrowParams)
    E(ItemWallTemplate)
    E(TrajectoryRules)
    E(LevelMap)
    E(LimbsMapping)
    E(LongRestCost)
    E(ManagedStatusVFX)
    E(MultiEffectInfo)
    E(OriginIntroEntity)
    E(Origin)
    E(PassiveList)
    E(PassiveVFX)
    E(ProgressionDescription)
    E(Progression)
    E(ProjectileDefault)
    E(Race)
    E(RandomCastOutcome)
    E(SkillList)
    E(SpellSoundTrajectory)
    E(SpellList)
    E(StatusSoundState)
    E(SurfaceCursorMessage)
    E(TadpolePower)
    E(TagSoundState)
    E(TooltipExtraText)
    E(TooltipUpcastDescription)
    E(TutorialEntries)
    E(TutorialModalEntries)
    E(TutorialUnifiedEntry)
    E(TutorialEvent)
    E(VFX)
    E(Voice)
    E(WeaponAnimationSet)
    E(WeightCategory)
    E(ShapeshiftRule)
    E(AnimationSetPriority)
    E(SpellMetaCondition)
    E(ScriptMaterialParameterOverride)
    E(ScriptMaterialPresetOverride)
    E(VisualLocatorAttachment)
    E(Flag)
    E(Tag)
    E(RulesetModifier)
    E(RulesetModifierOption)
    E(Ruleset)
    E(RulesetValue)
    E(RulesetSelectionPreset)
    E(OneTimeReward)
    E(AnimationShortNameCategory)
    E(AnimationShortName)
    E(PhotoModeBlueprintOverride)
    E(PhotoModeColourGrading)
    E(PhotoModeDecorFrame)
    E(PhotoModeEmoteAnimation)
    E(PhotoModeEmoteCollection)
    E(PhotoModeEmotePose)
    E(PhotoModeFaceExpressionCollection)
    E(PhotoModeFaceExpression)
    E(PhotoModeSticker)
    E(PhotoModeVignette)
    E(Max)
END_ENUM()

BEGIN_ENUM(ExtComponentType, uint32_t)
    E(ActionResources)
    E(ActionResourceSpendEvent)
    E(ActionResourceChangeResults)
    E(ActionResourceEvents)
    E(Armor)
    E(ArmorSetState)
    E(BaseHp)
    E(IsCharacter)
    E(Data)
    E(Detached)
    E(Health)
    E(Resistances)
    E(Hearing)
    E(SpellBook)
    E(Stats)
    E(StatusImmunities)
    E(SurfacePathInfluences)
    E(Use)
    E(Value)
    E(Weapon)
    E(PerItemSpellSource)
    E(Wielding)
    E(CustomStats)
    E(ActionResourceConsumeMultiplierBoost)
    E(Tag)
    E(SpellBookPrepares)
    E(Relation)
    E(Faction)
    E(CanInteract)
    E(CanSpeak)
    E(Origin)
    E(EocLevel)
    E(Classes)
    E(MaterialParameterOverride)
    E(OffStage)
    E(PickingState)
    E(Player)
    E(SimpleCharacter)
    E(Speaker)
    E(WeaponSet)
    E(DifficultyCheck)
    E(AttributeFlags)
    E(BodyType)
    E(ObjectSize)
    E(BaseStats)
    E(Expertise)
    E(HealBlock)
    E(Ruleset)
    E(RulesetModifiers)
    E(TimeFactor)
    E(SpatialGrid)
    E(SpatialGridCharacter)
    E(SpatialGridItem)

    E(Scene)
    E(SceneRoot)

    // Transform
    E(Transform)
    E(TranslateChanged)
    E(TranslateChangedEvent)
    E(GameplaySetTransform)
    E(InventoryMemberSetTransform)
    E(InventoryMemberSetTranslate)

    // Level
    E(Level)
    E(LevelRoot)
    E(LevelInstance)
    E(LevelInstanceState)
    E(LevelInstanceTempDestroyed)
    E(LevelUnloadEvent)
    E(LevelPrepareUnloadEvent)
    E(LevelUnloaded)
    E(LevelIsOwner)
    E(LevelPrepareUnloadBusy)
    E(LevelUnloadBusy)
    E(LevelInstanceLoad)
    E(LevelInstanceUnloading)
    E(LevelInstanceUnloaded)
    E(LevelInstanceLoaded)

    // Exp
    E(Experience)
    E(AvailableLevel)
    E(CanLevelUp)

    // Boosts
    E(BoostCondition)
    E(BoostsContainer)
    E(BoostInfo)

    // Server boosts
    E(ServerBoostBase)
    E(ServerStatusBoostsProcessed)
    E(BoostChangedEvent)
    E(BoostChangedEventsSingleton)
    E(BoostConditionalState)
    E(BoostProvider)
    E(StatusBoostsRefreshed)
    E(BoostBaseUpdated)

    // Stats
    E(ServerBaseData)
    E(ServerBaseSize)
    E(ServerBaseStats)
    E(ServerBaseWeapon)
    E(ServerBaseProficiency)
    E(ServerProficiencyGroupStats)

    // Stats events
    E(AbilityCheckEvent)
    E(EquipmentSlotChangedEvent)
    E(LevelChanged)
    E(SavingThrowRolledEvent)
    E(SkillCheckEvent)
    E(WeaponPropertiesChangedEvent)
    E(AttributeFlagsChangedEvent)
    E(ClassesChangedEvent)
    E(StatsAppliedEvent)

    // Shapeshift
    E(ShapeshiftState)
    E(ShapeshiftReplicatedChanges)
    E(ShapeshiftHealthReservation)
    E(ShapeshiftRecoveryAnimation)
    E(ShapeshiftSourceCache)
    E(ShapeshiftAnimation)
    E(ServerShapeshiftStates)

    // Multiplayer
    E(MultiplayerUser)
    E(MultiplayerHost)

    // Animations
    E(AnimationWaterfall)
    E(DynamicAnimationTags)
    E(TemplateAnimationSetOverride)
    E(AnimationTextKeyEventsSingleton)
    E(AnimationGameplayEventsSingleton)
    E(AnimationGameplayEvents)
    E(AnimationTextKeyEvents)
    E(AnimationTriggeredEvents)
    E(RemoveAnimationSetsRequest)
    E(RemoveAnimationSetsGameplayRequest)
    E(LoadAnimationSetRequest)
    E(LoadAnimationSetGameplayRequest)
    E(AnimationSetUpdateRequest)

    // Effects
    E(Effect)
    E(EffectCreate)
    E(AlwaysUpdateEffect)
    E(GameplayEffectSetTimeFactorRequests)
    E(GameplayVFXSetPlayTimeRequests)
    E(GameplayVFX)
    E(ClientEffectHandler)
    E(Decal)

    // Passives
    E(PassiveContainer)
    E(Passive)
    E(PassiveUsageCount)

    // Item components
    E(ItemDestroyed)
    E(ItemDye)
    E(MapMarkerStyle)
    E(ItemPortal)
    E(ItemDestroying)
    E(IsDoor)
    E(ExamineDisabled)
    E(ItemHasMoved)
    E(HasOpened)
    E(ItemInUse)
    E(ItemIsPoisoned)
    E(IsGold)
    E(IsItem)
    E(NewInInventory)
    E(ShouldDestroyOnSpellCast)

    // Item template components
    E(ActionType)
    E(ItemCanMove)
    E(ClimbOn)
    E(ItemTemplateDestroyed)
    E(InteractionDisabled)
    E(IsStoryItem)
    E(Ladder)
    E(WalkOn)
    E(UseAction)

    // Ownership
    E(OwnedAsLoot)
    E(OwneeCurrent)
    E(ServerDynamicLayerOwner)
    E(ServerIsCurrentOwner)
    E(ServerIsLatestOwner)
    E(ServerIsPreviousOwner)
    E(ServerIsOriginalOwner)
    E(ServerOwneeHistory)
    E(ServerOwneeRequest)

    // Inventory components
    E(InventoryMember)
    E(InventoryData)
    E(InventoryOwner)
    E(InventoryIsOwned)
    E(InventoryContainer)
    E(InventoryLocked)
    E(InventoryMemberTransform)
    E(InventoryStack)
    E(InventoryStackMember)
    E(InventoryTopOwner)
    E(InventoryWeight)
    E(TradeBuybackData)
    E(Wielded)
    E(WieldingHistory)
    E(CanBeInInventory)
    E(CannotBePickpocketed)
    E(CannotBeTakenOut)
    E(DropOnDeathBlocked)
    E(NewItemsInside)
    E(NonTradable)

    E(ServerInventoryContainerData)
    E(ServerInventoryGroupCheck)
    E(ServerShapeshiftEquipmentHistory)
    E(CharacterHasGeneratedTradeTreasure)
    E(ServerInventoryIsReplicatedWith)
    E(ReadyToBeAddedToInventory)
    E(HasGeneratedTreasure)

    E(InventoryPropertyCanBePickpocketed)
    E(ScriptPropertyCanBePickpocketed)
    E(InventoryPropertyIsDroppedOnDeath)
    E(ScriptPropertyIsDroppedOnDeath)
    E(InventoryPropertyIsTradable)
    E(ScriptPropertyIsTradable)

    // Party
    E(PartyView)
    E(PartyComposition)
    E(PartyPortals)
    E(PartyRecipes)
    E(PartyWaypoints)
    E(PartyMember)
    E(PartyFollower)
    E(CurrentlyFollowingParty)
    E(BlockFollow)
    E(Recruiter)
    E(ServerUserSnapshot)

    // Improvised Weapon
    E(ImprovisedWeaponWielded)
    E(ImprovisedWeaponWielding)
    E(CanBeWielded)

    // Pickup
    E(PickUpRequest)
    E(PickUpExecuting)

    // Escort
    E(EscortFollower)
    E(EscortLeader)
    E(EscortLeaderPriority)
    E(EscortMember)
    E(EscortStragglersTracker)
    E(EscortHasStragglers)

    // Camera components
    E(Camera)
    E(DefaultCameraBehavior)
    E(EffectCameraBehavior)
    E(GameCameraBehavior)
    E(CameraSelectorMode)
    E(CameraTarget)
    E(CameraCombatTarget)
    E(CameraCombatTargetRequests)
    E(CameraModeTracker)
    E(CameraInSelectorMode)
    E(CameraInSelectorWhileInactive)
    E(CameraSpellTracking)
    E(CameraPlatformTarget)
    E(CameraArriveWatcher)
    E(CameraClearScreenFadeRequestManual)
    E(CameraScreenFadeToRequestManual)

    // Camera photo mode
    E(PhotoModeCameraTransformRequests)
    E(PhotoModeCameraBehavior)
    E(PhotoModeExitScreenFadeClearRequests)
    E(PhotoModeExitScreenFadeCreateRequests)
    E(PhotoModeCameraEnterRequests)
    E(PhotoModeCameraDestructionRequests)
    E(PhotoModeCameraOriginalTransform)
    E(PhotoModeCameraInput)

    // Camp components
    E(CampChest)
    E(CampEndTheDayState)
    E(CampQuality)
    E(CampSettings)
    E(CampSupply)
    E(CampTotalSupplies)
    E(CampPresence)

    // Combat components
    E(IsInCombat)
    E(CombatDelayedFanfare)
    E(CombatIsThreatened)
    E(CombatParticipant)
    E(CombatState)
    E(TurnBased)
    E(TurnOrder)
    E(IsCombatPaused)
    E(ThreatRange)
    E(TurnOrderAssignEntityToUserRequest)
    E(TurnEndedEvent)
    E(TurnStartedEvent)
    E(TurnOrderSkipped)
    E(TurnOrderTimedOut)
    E(TurnSurfaceTeamSingleton)
    E(TurnSurfaceTracking)
    E(TurnRoundEndedEvent)

    // Death
    E(Death)
    E(DeathState)
    E(DeathType)
    E(DeadByDefault)
    E(Downed)
    E(DeadReckoning)
    E(DeadReckoningSync)
    E(ServerKiller)
    E(ServerDeathState)
    E(ServerDeathContinue)
    E(ServerDelayDeath)
    E(ServerDelayDeathCause)

    E(DeathApplyKnockedOut)
    E(DeathAssignEntityToUserRequest)
    E(DeathDeadByDefaultRequest)
    E(ServerDeathRequest)
    E(ServerModifyDelayDeathRequest)
    E(ServerResurrectedEvent)
    E(DiedEvent)
    E(DownedEvent)
    E(FallToProne)

    // Dummy
    E(DummyAnimationState)
    E(DummyAvailableAnimations)
    E(DummyCharacterVFXInitialization)
    E(DummiesCreatedSingleton)
    E(Dummy)
    E(DummyEquipmentVisualsState)
    E(DummyFootIKState)
    E(HasDummy)
    E(DummyMultiEffectCreateRequest)
    E(DummyOriginalTransform)
    E(DummySpellVFXInitialization)
    E(DummyStatusVFXInitialization)
    E(DummySplatter)
    E(DummyStoredCloth)
    E(DummyTransformRequestsSingleton)
    E(DummyUnsheath)
    E(DummyVFXEntities)
    E(DummyIsCopyingFullPose)
    E(DummyLoaded)

    // Photo mode
    E(PhotoModeDummyAnimationState)
    E(PhotoModeCameraOffset)
    E(PhotoModeCameraSavedTransform)
    E(PhotoModeCameraTilt)
    E(PhotoModeCameraTracking)
    E(PhotoModeDummyAnimationUpdateSingleton)
    E(PhotoModeDummyEquipmentSetupOneFrame)
    E(PhotoModeDummyEquipmentVisualUpdateSingleton)
    E(PhotoModeDummySplatterUpdateSingleton)
    E(PhotoModeDummyTransformUpdateSingleton)
    E(PhotoModeInvisibilityRequestSingleton)
    E(PhotoModeRequestedSingleton)
    E(PhotoModeSession)
    E(PhotoModeDummy)
    E(PhotoModeDummyTransform)
    E(PhotoModeDummyShowSplatter)
    E(PhotoModeDummyEquipmentVisual)
    E(PhotoModeCameraTransform)
    E(PhotoModeCapability)

    // Hit
    E(HitTarget)
    E(HitAttacker)
    E(HitWeapon)
    E(HitThrownObject)
    E(HitProxy)
    E(HitProxyOwner)
    E(HitReaction)
    E(HitMeta)
    E(HitLifetime)

    // Hit events
    E(HitAnimationRequest)
    E(HitResultEvent)
    E(HitJoinCombatRequest)
    E(UnresolvedHitNotification)
    E(HitNotification)
    E(HitNotificationRequest)

    // Identity
    E(Identity)
    E(IdentityState)
    E(OriginalIdentity)

    // Server combat components
    E(ServerCanStartCombat)
    E(ServerCombatGroupMapping)
    E(ServerEnterRequest)
    E(ServerFleeBlocked)
    E(ServerImmediateJoin)
    E(ServerCombatLeaveRequest)
    E(CombatSwitched)
    E(CombatFleeRequest)
    E(GlobalCombatRequests)
    E(CombatJoining)
    E(CombatLateJoinPenalty)
    E(CombatMerge)
    E(CombatSurfaceTeamSingleton)
    E(CombatantJoinEvent)
    E(CombatFleeSuccess)

    // Server combat events
    E(CombatantKilledEvent)
    E(CombatLeftEvent)
    E(CombatScheduledForDelete)
    E(CombatStartedEvent)
    E(DelayedFanfareRemovedDuringCombatEvent)
    E(CombatJoinInCurrentRoundFailedEvent)
    E(CombatJoinInCurrentRound)
    E(CombatRequestCompletedEvent)
    E(CombatSurprisedJoinRequest)
    E(CombatSurprisedStealthRequest)
    E(CombatThreatRangeChangedEvent)

    // FTB
    E(FTBParticipant)
    E(FTBZoneBlockReason)
    E(FTBRespect)
    E(FTBPaused)
    E(IsInFTB)

    // Server FTB
    E(FTBSurfaceTeamSingleton)
    E(FTBTimeFactorRequests)
    E(FTBTimeFactorResetRequests)
    E(FTBTurnBasedChangesRequest)
    E(FTBZone)
    E(FTBZoneInstigator)
    E(FTBTurnBased)

    // Server FTB events
    E(FTBModeChangedEvent)
    E(FTBPlayersTurnEndedEvent)
    E(FTBPlayersTurnStartedEvent)
    E(FTBRoundEndedEvent)

    // Ai
    E(ServerAiInterestedInItems)
    E(ServerAiInterestingItem)
    E(ServerAiArchetype)
    E(ServerAiModifiers)

    // Spell components
    E(SpellContainer)
    E(PlayerPrepareSpell)
    E(CCPrepareSpell)
    E(SpellModificationContainer)
    E(AddedSpells)
    E(AttackSpellOverride)
    E(ScriptedExplosion)
    E(SpellBookCooldowns)
    E(LearnedSpells)
    E(SpellAiConditions)
    E(OnDamageSpells)
    E(NewSpellsAddedEvent)
    E(SpellsLearnedEvent)
    E(SpellBookChanged)

    // Spell casting
    E(SpellCastAnimationInfo)
    E(SpellCastCache)
    E(SpellCastDataCache)
    E(SpellCastCanBeTargeted)
    E(SpellCastIsCasting)
    E(SpellCastInterruptResults)
    E(SpellCastMovement)
    E(SpellCastRolls)
    E(SpellCastState)
    E(SpellSyncTargeting)
    E(SpellCastOutcome)
    E(SpellCastExecutionTime)
        
    // Spell casting events
    E(SpellCastEvent)
    E(SpellCastFinishedEvent)
    E(SpellCastTextKeyEvent)
    E(SpellCastHitEvent)
    E(SpellCastDestroyEvent)
    E(SpellCastSpellRollAbortEvent)
    E(SpellCastTargetHitEvent)
    E(SpellCastTargetHitInterruptEvent)
    E(SpellCastTargetReactionEvent)
    E(SpellCastTargetsChangedEvent)
    E(SpellCastAnimationRequest)
    E(SpellCastCounteredEvent)
    E(SpellCastJumpStartEvent)
    E(SpellCastLogicExecutionStartEvent)
    E(SpellCastLogicExecutionEndEvent)
    E(SpellCastPrepareEndEvent)
    E(SpellCastPrepareStartEvent)
    E(SpellCastPreviewEndEvent)
    E(SpellCastThrowPickupPositionChangedEvent)

    // Server spell casting
    E(ServerSpellCastHitDelay)
    E(ServerSpellCastMovementInfo)
    E(ServerSpellCastRequests)
    E(ServerSpellCastPendingRequests)
    E(ServerSpellCastResponsible)
    E(ServerSpellClientInitiated)
    E(ServerSpellExternals)
    E(ServerSpellHitRegister)
    E(ServerSpellInterruptRequests)
    E(ServerSpellInterruptResults)
    E(ServerSpellCastMovement)
    E(ServerSpellCastState)
    E(ServerSpellCastCache)
    E(ServerSpellCastInterrupt)
    E(ServerSpellCastZoneRange)
    E(ServerSpellCastProjectilePathfindCache)
    E(ServerSpellCastUnsheathFallbackTimer)
    E(SpellCastInterruptsUsed)
    E(SpellCastMoveDuringCastUpdateEvent)
    E(SpellCastMovementAndPrecalculationEndEvent)
    E(SpellCastRequestTargetTracking)
    E(SpellCastUpdateTargetTracking)
    E(SpellCastWeaponSetChangeRequest)

    // Client spell casting
    E(ClientSpellCastCache)
    E(ClientSpellCastCachedAnimationRequests)
    E(ClientSpellCastEffectTimeFactorRequests)
    E(ClientSpellCastEffects)
    E(ClientSpellCastInterruptPauseRequests)
    E(ClientSpellCastMoveDuringCastUpdate)
    E(ClientSpellCastMovement)
    E(ClientSpellCastPlaySoundRequest)
    E(ClientSpellCastRolls)
    E(ClientSpellCastSetSoundSwitchesRequest)
    E(ClientSpellCastSharedToClientEntity)
    E(ClientSpellCastSoundImpactEvent)
    E(ClientSpellCastSounds)
    E(ClientSpellCastState)
    E(ClientSpellCastTargeting)
    E(ClientSpellCastTempSoundEventRequests)
    E(ClientSpellCastZoneRange)
    E(ClientSpellCastRollsChanged)

    // Concentration events
    E(ConcentrationChanged)
    E(ConcentrationDamageCheck)
    E(ConcentrationClearedEvent)

    // Interrupts
    E(InterruptActionState)
    E(InterruptConditionallyDisabled)
    E(InterruptContainer)
    E(InterruptData)
    E(InterruptDecision)
    E(InterruptPreferences)
    E(InterruptPrepared)
    E(InterruptZone)
    E(InterruptZoneParticipant)
    E(InterruptZoneSource)

    // Server interrupts
    E(ServerInterruptAddRemoveRequests)
    E(ServerInterruptActionRequests)
    E(ServerInterruptZoneRequests)
    E(ServerInterruptInitialParticipants)
    E(ServerInterruptTurnOrderInZone)
    E(ServerInterruptDataSingleton)
    E(ServerInterruptUsed)

    // Client interrupts
    E(ClientInterruptPlayerDecision)

    // Sight
    E(Sight)
    E(SightData)
    E(SightEntityViewshed)
    E(IgnoreSurfaces)
    E(ServerViewshedParticipant)
    E(ServerSightAggregatedData)
    E(ServerSightEntityLosCheckQueue)
    E(ServerAiGridViewshed)
    E(ServerLightLosCheckQueue)
    E(ServerAggregatedGameplayLightData)
    E(ServerSightEntityViewshedContentsChanged)
    E(ServerSightRangeChanged)
    E(ServerDarkvisionRangeChanged)
    E(ServerSightEventsEnabled)
    E(ServerSightIgnoreSurfacesChanged)
    E(ServerStealthRollCancel)
    E(ServerStealthRollRequest)

    // Navcloud
    E(NavcloudObstacle)
    E(NavcloudObstacleMetaData)
    E(NavcloudInRange)

    // Status
    E(StatusContainer)
    E(StatusCause)
    E(StatusID)
    E(StatusIncapacitated)
    E(StatusIndicateDarkness)
    E(StatusLifetime)
    E(StatusLoseControl)
    E(StatusVisualDisabled)

    E(ServerStatusCause)
    E(ServerStatus)
    E(ServerStatusOwnership)
    E(ServerStatusDifficultyModifiers)
    E(ServerStatusUnique)
    E(ServerStatusPerforming)
    E(ServerStatusRecoverable)
    E(ServerStatusActive)
    E(ServerStatusAddedFromSaveLoad)
    E(ServerStatusAura)
    E(ServerStatusAuraContainer)
    E(ServerStatusRequests)

    // Status events
    E(ServerAddedStatusAuraEffectEvent)
    E(ServerRemovedStatusAuraEffectEvent)
    E(ServerStatusAddEvent)
    E(ServerStatusApplyEvent)
    E(ServerStatusAttemptEvent)
    E(ServerStatusAttemptFailedEvent)
    E(ServerStatusDispelRollCheck)
    E(ServerStatusDownedChangedEvent)
    E(ServerStatusRefreshed)
    E(ServerStatusRemoveEvent)
    E(ServerStatusScheduledForDeletion)
    E(ServerStatusEvent)
    E(ServerStatusTurnStartEvent)
    E(ServerStatusUpdateTargetTracking)

    // Tadpole
    E(TadpoleTreeState)
    E(TadpolePowers)
    E(Tadpoled)
    E(HalfIllithid)
    E(FullIllithid)

    // Rest
    E(ShortRest)
    E(GlobalShortRestDisabled)
    E(GlobalLongRestDisabled)
    E(StoryShortRestDisabled)
    E(CanDoRest)
    E(LongRestInScriptPhase)
    E(LongRestState)
    E(LongRestTimeline)
    E(LongRestTimers)
    E(LongRestUsers)
    E(RestingEntities)

    // Server Rest
    E(ShortRestConsumeResources)
    E(ServerRestPendingType)
    E(ServerRestTypeChosenEvent)
    E(ServerShortRestResultEvent)

    // Character creation
    E(CCCompanionDefinition)
    E(CCDefinitionCommon)
    E(CCFullRespecDefinition)
    E(CCRespecDefinition)
    E(CCChangeAppearanceDefinition)
    E(CCCharacterDefinition)
    E(CCSessionCommon)
    E(CCState)
    E(CCLevelUp)
    E(CCLevelUpDefinition)
    E(CCFullRespec)
    E(CCRespec)
    E(CCCreation)
    E(CCDefinitionLevelUp)

    E(ClientCCDefinitionState)
    E(ClientCCDefinitionStateEx)
    E(ClientCCBaseDefinitionState)
    E(ClientCCCompanionDefinition)
    E(ClientCCLevelUpDefinition)
    E(ClientCCChangeAppearanceDefinition)
    E(ClientCCFullRespecDefinition)
    E(ClientCCDummyDefinition)

    E(ServerCCCustomIcon)
    E(ServerCCAppearanceVisualTag)
    E(ServerCCEquipmentSetRequest)
    E(ServerCCGod)
    E(ServerCCUpdates)
    E(ServerCCIsCustom)

    // Trigger
    E(TriggerArea)
    E(TriggerType)
    E(TriggerContainer)
    E(TriggerIsInsideOf)
    E(TriggerUpdatedContainer)
    E(TriggerUpdatedPhysics)

    // Server Trigger
    E(ServerStatsAreaTrigger)
    E(ServerSoundVolumeTrigger)
    E(ServerAIHintAreaTrigger)
    E(ServerCrowdCharacterTrigger)
    E(ServerChasmSeederTrigger)
    E(ServerTeleportTrigger)
    E(ServerCampRegionTrigger)
    E(ServerEocAreaTrigger)
    E(ServerStartTrigger)
    E(ServerAtmosphereTrigger)
    E(ServerCrimeAreaTrigger)
    E(ServerCrimeRegionTrigger)
    E(ServerEventTrigger)
    E(ServerFloorTrigger)
    E(ServerPortalTrigger)
    E(ServerRoomTrigger)
    E(ServerAvatarContainerTrigger)
    E(ServerCampChestTrigger)
    E(ServerChasmRegionTrigger)
    E(ServerExplorationTrigger)
    E(ServerMusicVolumeTrigger)
    E(ServerMusicVolumeTriggerState)
    E(ServerTimelineSceneTrigger)
    E(ServerBlockBronzeTimelinePlacementTrigger)
    E(ServerAiGridAreaTrigger)
    E(ServerRegionTrigger)
    E(ServerLightingTrigger)
    E(ServerEocPointTrigger)
    E(ServerTriggerEventConfig)
    E(ServerTriggerRegisteredFor)
    E(ServerRegistrationSettings)
    E(ServerUpdatedRegisteredFor)
    E(ServerUpdatedRegisteredForItems)
    E(ServerTriggerCachedLeaveEvents)
    E(ServerTriggerLoadedHandled)
    E(ServerTriggerWorldAutoTriggered)

    // Client Trigger
    E(ClientSoundVolumeTrigger)
    E(ClientAtmosphereTrigger)
    E(ClientFloorTrigger)
    E(ClientPortalTrigger)
    E(ClientRoomTrigger)
    E(ClientTimelineSceneTrigger)
    E(ClientRegionTrigger)
    E(ClientLightingTrigger)

    // Server ping
    E(ServerOsirisPingRequestSingleton)
    E(ServerPingCooldownSingleton)
    E(ServerPingRequestSingleton)

    // Lighting
    E(GameplayLight)
    E(Light)
    E(CombinedLight)
    E(CharacterLight)
    E(CharacterLightSingleton)
    E(ActiveCharacterLight)
    E(ServerGameplayLightEquipment)
    E(ServerGameplayLightChanges)

    // Anubis
    E(AnubisConfig)
    E(AnubisTree)
    E(AnubisTree2)
    E(AnubisRuntime)
    E(AnubisEnabled)
    E(Constellation)

    // Hotbar
    E(HotbarContainer)
    E(HotbarDecks)
    E(HotbarContainerKeyCreated)

    E(ServerHotbarOrder)
    E(ServerHotbarSlotsAdded)
    E(ServerHotbarSlotsRemoved)

    // Experimental components
    E(Background)
    E(God)
    E(Proficiency)
    E(ProficiencyGroup)
    E(AreaLevel)
    E(LevelUp)
    E(Floating)
    E(Voice)
    E(CustomIcon)
    E(CharacterCreationStats)
    E(Disarmable)
    E(IsSummon)
    E(SummonContainer)
    E(Stealth)
    E(ClientControl)
    E(IsGlobal)
    E(Savegame)
    E(SavegameLoaded)
    E(LootingState)
    E(Loot)
    E(SummonLifetime)
    E(Invisibility)
    E(Icon)
    E(OriginTag)
    E(OriginPassives)
    E(ClassTag)
    E(BackgroundTag)
    E(OriginAppearanceTag)
    E(VoiceTag)
    E(GodTag)
    E(BackgroundPassives)
    E(FleeCapability)
    E(IsInTurnBasedMode)
    E(ItemBoosts)
    E(ClientPaperdoll)
    E(AnimationSet)
    E(AnimationUpdate)
    E(AnimationBlueprint)
    E(CanModifyHealth)
    E(CanBeLooted)
    E(CanBeDisarmed)
    E(CanTriggerRandomCasts)
    E(CanDoActions)
    E(ActionUseConditions)
    E(CanMove)
    E(CanSense)
    E(Concentration)
    E(Darkness)
    E(DualWielding)
    E(GameObjectVisual)
    E(AppearanceOverride)
    E(CharacterCreationTemplateOverride)
    E(GravityDisabled)
    E(GravityDisabledUntilMoved)
    E(Avatar)
    E(HasExclamationDialog)
    E(Trader)
    E(CanTrade)
    E(IsFalling)
    E(HealMaxIncoming)
    E(HealMaxOutgoing)
    E(Ambushing)
    E(InteractionFilter)
    E(DisplayName)
    E(CustomName)
    E(Equipable)
    E(Cull)
    E(SceneAttach)
    E(IsSeeThrough)
    E(CustomIconsStorage)
    E(FogVolumeRequest)
    E(Race)
    E(CanTravel)
    E(Movement)
    E(ObjectInteraction)
    E(Pathing)
    E(PathingDistanceChanged)
    E(Steering)
    E(CanDeflectProjectiles)
    E(ActiveSkeletonSlots)
    E(Net)
    E(CharacterCreationAppearance)
    E(Active)
    E(Repose)
    E(OriginalTemplate)
    E(CanEnterChasm)
    E(DialogState)
    E(EncumbranceState)
    E(EncumbranceStats)
    E(Unsheath)
    E(UseSocket)
    E(UserAvatar)
    E(UserReservedFor)
    E(PauseExcluded)
    E(Pause)
    E(ServerUuidHistoryMapping)
    E(UuidHistoryTracked)
    E(FloorInfo)

    // Lock
    E(Lock)
    E(Key)
    E(LockAnimationState)
    E(LockLifetime)
    E(ServerLockpickingState)

    // Timeline
    E(TLPreviewDummy)
    E(TimelineActorData)
    E(TimelineData)
    E(SyncedTimelineActorControl)
    E(SyncedTimelineControl)
    E(ClientTimelineActorControl)
    E(ClientTimelineControl)

    // Sound
    E(Sound)
    E(SoundMaterial)
    E(SoundRoomCurrentState)
    E(SkeletonSoundObjects)
    E(SoundOcclusionData)
    E(SoundActivated)
    E(SoundUsesTransform)

    // Physics
    E(Physics)
    E(StaticPhysics)
    E(PhysicsLoad)
    E(PhysicsStreamLoad)
    E(PhysicsLoaderRegistered)
    E(PhysicsPathLoadDesciption)
    E(PhysicsResourceLoadDesciption)
    E(PhysicsLoaded)
    E(PhysicsReloadOneFrame)

    // Visuals
    E(Visual)
    E(VisualLoadDescription)
    E(VisualLoad)
    E(VisualStream)
    E(VisualStreamLoad)
    E(VisualLoadRequests)
    E(VisualChangeRequest)
    E(VisualAttachRequest)
    E(VisualLoaded)
    E(VisualChangedEvent)

    // Instancing
    E(InstancingGroup)
    E(InstancingGroupVisual)
    E(InstancingBatch)
    E(InstancingBatchModifyOneFrame)
    E(InstancingBatchInitialized)
    E(InstancingBatchVisualReloadedOneFrame)

    // HLOD
    E(HLOD)
    E(HLODChild)

    // Misc visual
    E(Bound)
    E(LocalBound)
    E(Occlusion)

    // Clustering
    E(ClusterBound)
    E(ClusterBoundMax)
    E(ClusterChild)
    E(ClusterChildIndexed)
    E(ClusterContainer)
    E(ClusterDistMin)
    E(ClusterDistMax)
    E(ClusterPositionX)
    E(ClusterPositionY)
    E(ClusterPositionZ)
    E(ClusterRadius)
    E(Cluster)
    E(ClusterAttachRequest)

    // Fade
    E(FadeChildrenAdded)
    E(FadeChildrenRemoved)
    E(FadeableObstructionHierarchy)
    E(FadeGroup)
    E(FadeGroupMapping)
    E(FadeableObstruction)
    E(FadeableObstructionMapping)

    E(BackgroundGoals)

    E(CalendarDaysPassed)
    E(CalendarStartingDate)

    // Approval
    E(ApprovalRatings)
    E(ServerRatingsChanged)

    // Attitude
    E(AttitudesToPlayers)

    // Equipment
    E(DisabledEquipment)
    E(EquipmentVisual)
    E(ClientEquipmentVisuals)
    E(ClientEquipmentVisibilityState)
    E(ClientVisualsDesiredState)

    // Splatter
    E(SplatterBaseState)
    E(SplatterState)
    E(SplatterSweatChange)

    // Progression
    E(ProgressionContainer)
    E(ProgressionMeta)
    E(ProgressionChangedContainers)
    E(ProgressionLevelUpChanged)
    E(ProgressionAbilityImprovements)
    E(ProgressionFeat)
    E(ProgressionSpells)
    E(ProgressionSkills)
    E(ProgressionPassives)
    E(ProgressionReplicatedFeat)

    // Through
    E(CanSeeThrough)
    E(CanShootThrough)
    E(CanWalkThrough)
    E(ShootThroughType)

    // Experimental server components
    E(ServerRecruitedBy)
    E(ServerGameTimer)
    E(ServerExperienceGaveOut)
    E(ServerReplicationDependency)
    E(ServerIsUnsummoning)
    E(ServerActivationGroupContainer)
    E(ServerAnubisTag)
    E(ServerDialogTag)
    E(ServerDisplayNameList)
    E(ServerIconList)
    E(ServerRaceTag)
    E(ServerTemplateTag)
    E(ServerBoostTag)
    E(ServerSafePosition)
    E(ServerAnubisExecutor)
    E(ServerLeader)
    E(ServerBreadcrumb)
    E(ServerPickpocket)
    E(ServerReplicationDependencyOwner)
    E(HistoryTargetUUID)
    E(ServerPeersInRange)
    E(ServerDisarmAttempt)
    E(ServerUnsheathSpellAnimationLifetime)
    E(ServerUnsheath)
    E(ServerUnsheathScriptOverride)
    E(ServerIsLightBlocker)
    E(ServerIsVisionBlocker)
    E(ServerDarknessActive)
    E(ServerInventoryItemDataPopulated)
    E(ServerVariableManager)
    E(IsMarkedForDeletion)
    E(JumpFollow)

    // Server surfaces
    E(ServerSurface)
    E(ServerSurfaceLevelLoaded)
    E(ServerSurfaceCreatedEvent)
    E(ServerSurfaceRemovedEvent)
    E(ServerSurfaceActionFinishedEvent)
    E(ServerSurfaceActionGrownEvent)

    // Server events
    E(ServerTemplateChangedEvent)
    E(ServerTemplateTransformedEvent)
    E(TagsChangedEvent)

    // Summons
    E(SummonCreatedEvent)
    E(SummonAddConcentrationRequest)
    E(SummonAddToExistingConcentrationRequest)
    E(SummonAttachToProjectileRequest)
    E(SummonOwnerSetEvent)
    E(SummonPlaceInInventoryRequest)
    E(SummonSetLifetimeRequest)
    E(SummonDespawnRequest)
    E(SummonExpiredRequest)
    E(SummonLateJoinPenalty)

    // Passives
    E(ServerToggledPassives)
    E(ServerPassiveBoosts)
    E(ServerPassiveBase)
    E(ServerPassivePersistentData)
    E(ServerScriptPassives)

    // Passives events
    E(PassiveRequestTargetTracking)
    E(PassiveUpdateTargetTracking)
    E(PassiveConditionalRollInterruptEvent)
    E(PassivesUpdatedEvent)
    E(PasssiveUsageCountIncrementedEvent)

    E(Construction)
    E(ConstructionTile)
    E(ConstructionFilling)

    E(ServerCharacter)
    E(ServerItem)

    // Projectile
    E(ProjectileSource)
    E(ServerProjectile)
    E(ServerProjectileAttachment)
    E(ServerProjectileSpell)
    E(ServerProjectileInitialization)
    E(ServerProjectileCache)
    E(ServerOsirisTag)

    // Projectile events
    E(ProjectileImpactEvent)
    E(ProjectileRequestTargetTracking)
    E(ProjectileUpdateTargetTracking)
    E(ProjectileSplitThrowableObjectRequest)

    // Custom Icon
    E(ClientCharacterIconRequest)
    E(ClientCharacterIconResult)

    E(ClientCharacter)
    // Not mapped yet
    // E(ClientItem)
    // E(ClientProjectile)
    E(Scenery)

    // Rolls
    E(RequestedRoll)
    E(RollInProgress)
    E(ServerRollFinishedEvent)
    E(RollModifiers)
    E(ServerRollInProgress)
    E(ServerRollStartRequest)
    E(ServerRollStartSpellRequest)
    E(ServerRollStreams)

    // Boost components
    E(ArmorClassBoost)
    E(AbilityBoost)
    E(RollBonusBoost)
    E(AdvantageBoost)
    E(ActionResourceValueBoost)
    E(CriticalHitBoost)
    E(AbilityFailedSavingThrowBoost)
    E(ResistanceBoost)
    E(WeaponDamageResistanceBoost)
    E(ProficiencyBonusOverrideBoost)
    E(JumpMaxDistanceMultiplierBoost)
    E(HalveWeaponDamageBoost)
    E(UnlockSpellBoost)
    E(SourceAdvantageBoost)
    E(ProficiencyBonusBoost)
    E(ProficiencyBoost)
    E(IncreaseMaxHPBoost)
    E(ActionResourceBlockBoost)
    E(StatusImmunityBoost)
    E(UseBoosts)
    E(CannotHarmCauseEntityBoost)
    E(TemporaryHPBoost)
    E(WeightBoost)
    E(WeightCategoryBoost)
    E(FactionOverrideBoost)
    E(ActionResourceMultiplierBoost)
    E(BlockRegainHPBoost)
    E(InitiativeBoost)
    E(DarkvisionRangeBoost)
    E(DarkvisionRangeMinBoost)
    E(DarkvisionRangeOverrideBoost)
    E(AddTagBoost)
    E(IgnoreDamageThresholdMinBoost)
    E(SkillBoost)
    E(WeaponDamageBoost)
    E(NullifyAbilityBoost)
    E(RerollBoost)
    E(DownedStatusBoost)
    E(WeaponEnchantmentBoost)
    E(GuaranteedChanceRollOutcomeBoost)
    E(AttributeBoost)
    E(GameplayLightBoost)
    E(DualWieldingBoost)
    E(SavantBoost)
    E(MinimumRollResultBoost)
    E(CharacterWeaponDamageBoost)
    E(ProjectileDeflectBoost)
    E(AbilityOverrideMinimumBoost)
    E(ACOverrideFormulaBoost)
    E(FallDamageMultiplierBoost)
    E(ActiveCharacterLightBoost)
    E(WeaponAttackTypeOverrideBoost)
    E(WeaponDamageDieOverrideBoost)
    E(CarryCapacityMultiplierBoost)
    E(WeaponPropertyBoost)
    E(WeaponAttackRollAbilityOverrideBoost)
    E(SightRangeAdditiveBoost)
    E(SightRangeMinimumBoost)
    E(SightRangeMaximumBoost)
    E(SightRangeOverrideBoost)
    E(MovementSpeedLimitBoost)
    E(UnlockSpellVariantBoost)
    E(DetectCrimesBlockBoost)
    E(BlockAbilityModifierFromACBoost)
    E(ScaleMultiplierBoost)
    E(DamageReductionBoost)
    E(ReduceCriticalAttackThresholdBoost)
    E(PhysicalForceRangeBonusBoost)
    E(ObjectSizeBoost)
    E(ObjectSizeOverrideBoost)
    E(AiArchetypeOverrideBoost)
    E(ExpertiseBonusBoost)
    E(EntityThrowDamageBoost)
    E(WeaponDamageTypeOverrideBoost)
    E(MaximizeHealingBoost)
    E(DamageBonusBoost)
    E(AdvanceSpellsBoost)
    E(SpellResistanceBoost)
    E(WeaponAttackRollBonusBoost)
    E(SpellSaveDCBoost)
    E(RedirectDamageBoost)
    E(CanSeeThroughBoost)
    E(CanShootThroughBoost)
    E(CanWalkThroughBoost)
    E(MonkWeaponDamageDiceOverrideBoost)
    E(HorizontalFOVOverrideBoost)
    E(CharacterUnarmedDamageBoost)
    E(ActionResourceReplenishTypeOverrideBoost)
    E(ActionResourcePreventReductionBoost)
    E(AttackSpellOverrideBoost)
    E(LockBoost)
    E(IgnorePointBlankDisadvantageBoost)
    E(CriticalHitExtraDiceBoost)
    E(DodgeAttackRollBoost)
    E(GameplayObscurityBoost)
    E(MaximumRollResultBoost)
    E(UnlockInterruptBoost)
    E(JumpMaxDistanceBonusBoost)
    E(ArmorAbilityModifierCapOverrideBoost)
    E(IgnoreResistanceBoost)
    E(ConcentrationIgnoreDamageBoost)
    E(IgnoreLowGroundPenaltyBoost)
    E(IgnoreSurfaceCoverBoost)
    E(ProficiencyBonusIncreaseBoost)
    E(DamageTakenBonusBoost)
    E(ReceivingCriticalDamageOnHitBoost)

    E(Uuid)

    // Special engine components
    E(UuidToHandleMapping)

    E(Max)
END_ENUM()

BEGIN_BITMASK(CanMoveFlags, uint16_t)
    EV(CanMove, 1)
    EV(CanBeTeleported, 2)
    EV(CanFollow, 4)
    EV(CanWorldClimb, 8)
    EV(CanWorldDrop, 0x10)
END_ENUM()

BEGIN_BITMASK(CanInteractFlags, uint16_t)
    EV(CanInteract, 1)
    EV(CanUsePortals, 2)
    EV(CanUseLadder, 4)
    EV(CanUseFlag8, 8)
END_ENUM()

BEGIN_BITMASK(CanDoActionsFlags, uint16_t)
    EV(CanCastSpells, 1)
    EV(CanAttack, 2)
    EV(CanDoFlag4, 4)
    EV(CanCastRituals, 8)
    EV(CanPickpocket, 0x10)
    EV(CanLockpick, 0x20)
    EV(CanDisarmTrap, 0x40)
END_ENUM()

BEGIN_BITMASK(ConstructionTileFlags, uint8_t)
    EV(WalkOn, 1)
    EV(Climbable, 2)
    EV(ShootThrough, 4)
    EV(CanSeeThrough, 8)
    EV(WalkThrough, 0x10)
    EV(ClickThrough, 0x20)
    EV(SeeThrough, 0x40)
END_ENUM()

BEGIN_BITMASK_NS(esv, StatusFlags, ServerStatusFlags, uint8_t)
    EV(ForceStatus, 1)
    EV(ForceFailStatus, 2)
    EV(IgnoreStacking, 4)
    EV(RequestClientSync, 8)
    EV(Loaded, 0x10)
    EV(RequestDelete, 0x20)
    EV(RequestDeleteAtTurnEnd, 0x40)
    EV(Started, 0x80)
END_ENUM_NS()

BEGIN_BITMASK_NS(esv, StatusFlags2, ServerStatusFlags2, uint8_t)
    EV(KeepAlive, 1)
    EV(DontTickWhileOnSurface, 2)
    EV(IsOnSourceSurface, 4)
    EV(IsFromItem, 8)
    EV(IsChanneled, 0x10)
    EV(IsLifeTimeSet, 0x20)
    EV(InitiateCombat, 0x40)
    EV(Influence, 0x80)
END_ENUM_NS()


BEGIN_BITMASK_NS(esv, StatusFlags3, ServerStatusFlags3, uint8_t)
    EV(IsUnique, 1)
    EV(NotifiedPlanManager, 2)
    EV(DisableImmunityOverhead, 4)
    EV(StatusFlags3_0x08, 8)
    EV(StatusFlags3_0x10, 0x10)
    EV(StatusFlags3_0x20, 0x20)
    EV(StatusFlags3_0x40, 0x40)
    EV(StatusFlags3_0x80, 0x80)
END_ENUM_NS()


BEGIN_BITMASK_NS(esv, StatusFlags4, ServerStatusFlags4, uint8_t)
    EV(BringIntoCombat, 1)
    EV(IsHostileAct, 2)
    EV(StatusFlags4_0x04, 4)
    EV(IsInvulnerable, 8)
    EV(IsInvulnerableVisible, 0x10)
    EV(CausedByEquipment, 0x20)
    EV(CauseIsOrigin, 0x40)
    EV(StatusFlags4_0x80, 0x80)
END_ENUM_NS()


BEGIN_BITMASK_NS(esv, StatusFlags5, ServerStatusFlags5, uint8_t)
    EV(HasTriedEntering, 1)
END_ENUM_NS()


BEGIN_ENUM_NS(esv, DestroyStatusRequestType, ServerDestroyStatusRequestType, uint8_t)
    EV(StatusType, 0)
    EV(StatusID, 1)
    EV(StatusEntity, 3)
    EV(StatusHandle, 4)
    EV(SourceEquippedItem, 5)
    EV(RestingEntityInventory, 6)
    EV(Death, 7)
    EV(Cause, 8)
    EV(DifficultyStatus, 9)
END_ENUM_NS()


BEGIN_BITMASK_NS(ecl, StatusFlags, ClientStatusFlags, uint8_t)
    EV(RequestDelete, 1)
    EV(Started, 2)
    EV(WasEnteredDuringSave, 4)
    EV(KeepAlive, 8)
    EV(DontTickWhileOnSurface, 0x10)
    EV(IsOnSourceSurface, 0x20)
    EV(VisualsCreated, 0x40)
    EV(HasSounds, 0x80)
END_ENUM_NS()

BEGIN_BITMASK_NS(ecl, StatusFlags2, ClientStatusFlags2, uint8_t)
    EV(HasLoopSound, 1)
    EV(Ticked, 2)
    EV(IsSourceDependent, 4)
END_ENUM_NS()

// Persistent flags: 0x2414170022110
// AI flags: 0x4000070000060
BEGIN_BITMASK_NS(esv, CharacterFlags, ServerCharacterFlags, uint64_t)
    EV(IsPlayer, 0x01)
    EV(Multiplayer, 0x02)
    EV(InParty, 0x04)
    EV(HostControl, 0x08)
    EV(Activated, 0x10)
    EV(OffStage, 0x20)
    EV(Dead, 0x40)
    EV(HasOwner, 0x80)
    EV(InDialog, 0x100)
    EV(Summon, 0x200)
    EV(Invulnerable, 0x400)
    EV(CannotDie, 0x800)
    EV(StoryNPC, 0x1000)
    EV(CharacterControl, 0x2000)
    EV(Loaded, 0x4000)
    EV(Unknown8000, 0x8000)
    EV(Invisible, 0x10000)
    EV(StoryControlled, 0x20000)
    EV(FindValidPositionOnActivate, 0x40000)
    EV(CannotAttachToGroups, 0x80000)
    EV(CharCreationInProgress, 0x100000)
    EV(CharacterCreationFinished, 0x200000)
    EV(Floating, 0x400000)
    EV(FightMode, 0x800000)
    EV(SpotSneakers, 0x1000000)
    EV(Temporary, 0x2000000)
    EV(CannotMove, 0x4000000)
    EV(CannotRun, 0x8000000)
    EV(WalkThrough, 0x10000000)
    EV(CoverAmount, 0x20000000)
    EV(CanShootThrough, 0x40000000)
    EV(Unknown80000000, 0x80000000)
    EV(DontCacheTemplate, 0x100000000)
    EV(ReservedForDialog, 0x200000000)
    EV(PartyFollower, 0x400000000)
    EV(DisableWaypointUsage, 0x800000000)
    EV(IgnoresTriggers, 0x1000000000)
    EV(Totem, 0x2000000000)
    EV(IsTrading, 0x4000000000)
    EV(SteeringEnabled, 0x8000000000)
    EV(Unknown10000000000, 0x10000000000)
    EV(IsCompanion_M, 0x20000000000)
    EV(Unknown40000000000, 0x40000000000)
    EV(Deactivated, 0x80000000000)
    EV(CustomLookEnabled, 0x100000000000)
    EV(IsHuge, 0x200000000000)
    EV(DisableCulling, 0x400000000000)
    EV(DoNotFaceFlag, 0x800000000000)
    EV(NeedsMakePlayerUpdate, 0x1000000000000)
    EV(GMReroll, 0x2000000000000)
    EV(Detached, 0x4000000000000)
    EV(RequestStartTurn, 0x8000000000000)
    EV(DeferredRemoveEscapist, 0x10000000000000)
    EV(LevelTransitionPending, 0x20000000000000)
    EV(RegisteredForAutomatedDialog, 0x40000000000000)
    EV(Unknown80000000000000, 0x80000000000000)
END_ENUM_NS()


BEGIN_BITMASK_NS(esv, CharacterFlags2, ServerCharacterFlags2, uint8_t)
    EV(Global, 0x01)
    EV(TreasureGeneratedForTrader, 0x02)
    EV(Trader, 0x04)
    EV(IsResurrected, 0x08)
    EV(TeleportingPartyHack, 0x10)
    EV(IsPet, 0x20)
    EV(IsLeveledByStory, 0x40)
    EV(CanGossip, 0x80)
END_ENUM_NS()


BEGIN_BITMASK_NS(esv, CharacterFlags3, ServerCharacterFlags3, uint8_t)
    EV(NeedsPlacementSnapping, 0x01)
    EV(CrimeWarningsEnabled, 0x02)
    EV(SpreadActivation, 0x08)
    EV(TreasureEquip, 0x10)
END_ENUM_NS()


BEGIN_BITMASK_NS(ecl, CharacterFlags, ClientCharacterFlags, uint64_t)
    EV(IsPlayer, 1)
    EV(Active, 4)
    EV(InDialog, 0x40)
    EV(Climbing, 0x200)
    EV(HasCharacterLight, 0x2000)
    EV(VisibleOffStage, 0x4000000)
    EV(Airborne, 0x4000000000000)
END_ENUM_NS()

BEGIN_BITMASK_NS(ecl, CharacterFlags2, ClientCharacterFlags2, uint8_t)
    EV(PickpocketRequested, 0x02)
    EV(Pickpocketing, 0x04)
    EV(LootingRequested, 0x08)
    EV(CustomVisuals, 0x10)
    EV(ReloadingVisuals, 0x20)
    EV(RemovingFromLevel, 0x80)
END_ENUM_NS()

BEGIN_BITMASK_NS(ecl, CharacterFlags3, ClientCharacterFlags3, uint8_t)
    EV(HasDialog, 0x01)
    EV(WasInputControllerActive, 0x02)
    EV(HasCustomVisualAfterDeath, 0x10)
    EV(CanTrade, 0x20)
END_ENUM_NS()

BEGIN_BITMASK_NS(ecl, SceneryFlags, SceneryFlags, uint16_t)
    EV(WalkThrough, 0x01)
    EV(CanClimbOn, 0x02)
    EV(CanWalkOn, 0x04)
    EV(CanShootThrough, 0x08)
    EV(CanSeeThrough, 0x10)
    EV(Wadable, 0x20)
    EV(CanSeeThrough2, 0x40)
    EV(CollideWithCamera, 0x80)
    EV(AllowCameraMovement, 0x100)
    EV(IsBlocker, 0x200)
END_ENUM_NS()

BEGIN_ENUM_NS(ecl::camera, TargetRequestType, CameraTargetRequestType, uint8_t)
    EV(SetPlayerTargets, 0)
    EV(Follow, 1)
    EV(Projectile, 2)
    EV(Spell, 3)
    EV(Status, 4)
    EV(Restore, 5)
    EV(ClearTargets, 6)
    EV(DeathAnimation, 7)
    EV(CombatStarted, 8)
    EV(PlayerTurn, 9)
    EV(CharacterAssign, 10)
    EV(Swarm, 11)
    EV(DebugPos, 12)
END_ENUM_NS()

BEGIN_BITMASK_NS(esv, ItemFlags, ServerItemFlags, uint64_t)
    EV(Activated, 0x01)
    EV(OffStage, 0x02)
    EV(CanBePickedUp, 0x04)
    EV(CanBeMoved, 0x08)
    EV(WalkOn, 0x10)
    EV(WalkThrough, 0x20)
    EV(NoCover, 0x40)
    EV(CanShootThrough, 0x80)
    EV(CanUse, 0x100)
    EV(InteractionDisabled, 0x200)
    EV(TransformChanged, 0x400)
    EV(Destroyed, 0x800)
    EV(IsMoving, 0x1000)
    EV(CanOnlyBeUsedByOwner, 0x2000)
    EV(LoadedTemplate, 0x4000)
    EV(IsDoor, 0x8000)
    EV(Invisible, 0x10000)
    EV(StoryItem, 0x20000)
    EV(ClientSync1, 0x40000)
    EV(WakePhysics, 0x80000)
    EV(Summon, 0x100000)
    EV(IsContainer, 0x200000)
    EV(HideHP, 0x400000)
    EV(LuckyFind, 0x800000)
    EV(FreezeGravity, 0x1000000)
    EV(ForceSync, 0x2000000)
    EV(Known, 0x4000000)
    EV(Invisible2, 0x8000000)
    EV(DisableSync, 0x10000000)
    EV(IsLadder, 0x20000000)
    EV(ReservedForDialog, 0x40000000)
    EV(ForceAffectedByAura, 0x80000000)
    EV(DisableInventoryView80, 0x100000000)
    EV(DontAddToHotbar, 0x200000000)
    EV(PositionChanged, 0x400000000)
    EV(Totem, 0x800000000)
    EV(Destroy, 0x2000000000)
    EV(GMFolding, 0x4000000000)
    EV(InUse, 0x8000000000)
    EV(Sticky, 0x10000000000)
    EV(IsSecretDoor, 0x20000000000)
    EV(Floating, 0x40000000000)
    EV(IsSurfaceBlocker, 0x80000000000)
    EV(IsSurfaceCloudBlocker, 0x100000000000)
    EV(Invulnerable2, 0x200000000000)
    EV(ForceClientSync, 0x400000000000)
    EV(InPartyInventory, 0x800000000000)
    EV(SourceContainer, 0x1000000000000)
    EV(Frozen, 0x2000000000000)
    EV(LevelTransitionPending, 0x4000000000000)
    EV(TeleportOnUse, 0x8000000000000)
    EV(InAutomatedDialog, 0x10000000000000)
    EV(PinnedContainer, 0x20000000000000)
    EV(CanClimbOn, 0x40000000000000)
END_ENUM_NS()

BEGIN_BITMASK_NS(esv, ItemFlags2, ServerItemFlags2, uint8_t)
    EV(UnsoldGenerated, 0x01)
    EV(Global, 0x02)
    EV(TreasureGenerated, 0x08)
    EV(UnEquipLocked, 0x10)
    EV(UseRemotely, 0x20)
END_ENUM_NS()


BEGIN_ENUM(BoostType, uint8_t)
    E(AC)
    E(Ability)
    E(RollBonus)
    E(Advantage)
    E(ActionResource)
    E(CriticalHit)
    E(AbilityFailedSavingThrow)
    E(Resistance)
    E(WeaponDamageResistance)
    E(ProficiencyBonusOverride)
    E(ActionResourceOverride) // Has no component
    E(AddProficiencyToAC) // Has no component
    EV(JumpMaxDistanceMultiplier, 12)
    E(AddProficiencyToDamage) // Has no component
    EV(ActionResourceConsumeMultiplier, 14)
    E(BlockVerbalComponent) // Has no component
    E(BlockSomaticComponent) // Has no component
    EV(HalveWeaponDamage, 17)
    EV(UnlockSpell, 19)
    EV(SourceAdvantageOnAttack, 20)
    EV(ProficiencyBonus, 21)
    E(BlockSpellCast) // Has no component
    EV(Proficiency, 23)
    E(SourceAllyAdvantageOnAttack)
    E(IncreaseMaxHP)
    E(ActionResourceBlock)
    E(StatusImmunity)
    E(UseBoosts)
    E(CannotHarmCauseEntity)
    E(TemporaryHP)
    E(Weight)
    E(WeightCategory)
    E(FactionOverride)
    E(ActionResourceMultiplier)
    E(BlockRegainHP)
    E(Initiative)
    E(DarkvisionRange)
    E(DarkvisionRangeMin)
    E(DarkvisionRangeOverride)
    E(Tag)
    E(IgnoreDamageThreshold)
    E(Skill)
    E(WeaponDamage)
    E(NullifyAbilityScore)
    E(IgnoreFallDamage)
    EV(Reroll, 46)
    EV(DownedStatus, 47)
    E(Invulnerable) // Has no component
    EV(WeaponEnchantment, 49)
    EV(GuaranteedChanceRollOutcome, 50)
    EV(Attribute, 51)
    E(IgnoreLeaveAttackRange) // Has no component
    EV(GameplayLight, 53)
    E(DialogueBlock)
    E(DualWielding)
    EV(Savant, 56)
    EV(MinimumRollResult, 57)
    E(Lootable) // Has no component
    EV(CharacterWeaponDamage, 59)
    E(ProjectileDeflect)
    E(AbilityOverrideMinimum)
    E(ACOverrideFormula)
    E(FallDamageMultiplier)
    EV(ActiveCharacterLight, 64)
    E(Invisibility) // Has no component
    E(TwoWeaponFighting) // Has no component
    EV(WeaponAttackTypeOverride, 67)
    E(WeaponDamageDieOverride)
    E(CarryCapacityMultiplier)
    E(WeaponProperty)
    EV(WeaponAttackRollAbilityOverride, 71)
    E(BlockTravel) // Has no component
    E(BlockGatherAtCamp) // Has no component
    E(BlockAbilityModifierDamageBonus) // Has no component
    E(VoicebarkBlock) // Has no component
    E(HiddenDuringCinematic) // Has no component
    EV(SightRangeAdditive, 77)
    E(SightRangeMinimum)
    E(SightRangeMaximum)
    EV(SightRangeOverride, 80)
    E(CannotBeDisarmed) // Has no component
    EV(MovementSpeedLimit, 82)
    E(NonLethal) // Has no component
    EV(UnlockSpellVariant, 84)
    E(DetectDisturbancesBlock)
    E(BlockAbilityModifierFromAC)
    EV(ScaleMultiplier, 87)
    E(CriticalDamageOnHit) // Has no component
    EV(DamageReduction, 89)
    E(ReduceCriticalAttackThreshold)
    E(PhysicalForceRangeBonus)
    E(ObjectSize)
    EV(ObjectSizeOverride, 93)
    E(ItemReturnToOwner) // Has no component
    EV(AiArchetypeOverride, 95)
    E(ExpertiseBonus)
    E(EntityThrowDamage)
    E(WeaponDamageTypeOverride)
    EV(MaximizeHealing, 99)
    E(IgnoreEnterAttackRange) // Has no component
    EV(DamageBonus, 101)
    E(Detach) // Has no component
    E(ConsumeItemBlock) // Has no component
    EV(AdvanceSpells, 104)
    E(SpellResistance)
    E(WeaponAttackRollBonus)
    E(SpellSaveDC)
    E(RedirectDamage)
    E(CanSeeThrough)
    E(CanShootThrough)
    EV(CanWalkThrough, 111)
    E(MonkWeaponAttackOverride) // Has no component
    EV(MonkWeaponDamageDiceOverride, 113)
    E(IntrinsicSummonerProficiency) // Has no component
    EV(HorizontalFOVOverride, 115)
    EV(CharacterUnarmedDamage, 116)
    E(UnarmedMagicalProperty) // Has no component
    EV(ActionResourceReplenishTypeOverride, 118)
    E(AreaDamageEvade) // Has no component
    EV(ActionResourcePreventReduction, 120)
    EV(AttackSpellOverride, 121)
    EV(Lock, 122)
    E(NoAOEDamageOnLand) // Has no component
    EV(IgnorePointBlankDisadvantage, 124)
    E(CriticalHitExtraDice)
    E(DodgeAttackRoll)
    E(GameplayObscurity)
    E(MaximumRollResult)
    EV(UnlockInterrupt, 129)
    E(IntrinsicSourceProficiency) // Has no component
    EV(JumpMaxDistanceBonus, 131)
    E(ArmorAbilityModifierCapOverride)
    E(IgnoreResistance)
    EV(ConcentrationIgnoreDamage, 134)
    E(LeaveTriggers) // Has no component
    EV(IgnoreLowGroundPenalty, 136)
    E(IgnoreSurfaceCover)
    E(EnableBasicItemInteractions) // Has no component
    E(SoundsBlocked) // Has no component
    E(ProficiencyBonusIncrease)
    E(NoDamageOnThrown)
    E(DamageTakenBonus)
    E(ReceivingCriticalDamageOnHit)
    E(Sentinel)
END_ENUM()

BEGIN_BITMASK_NS(ecs, EntityChangeFlags, ECSEntityChangeFlags, uint8_t)
    EV(Create, 1)
    EV(Destroy, 2)
    EV(Immediate, 4)
    EV(Dead, 8)
    EV(Ignore, 0x10)
END_ENUM_NS()

BEGIN_BITMASK_NS(ecs, ComponentChangeFlags, ECSComponentChangeFlags, uint8_t)
    EV(Create, 1)
    EV(Destroy, 2)
    EV(Replicate, 4)
    EV(OneFrame, 8)
    EV(ReplicatedComponent, 0x10)
    EV(Modify, 0x20)
END_ENUM_NS()

BEGIN_ENUM(LevelInstanceState, uint8_t)
    EV(Uninitialized, 0)
    EV(Load, 1)
    EV(LoadDependencies, 2)
    EV(Instantiate, 3)
    EV(Instantiated, 4)
END_ENUM()

BEGIN_BITMASK(InterruptInteractionTypes, uint8_t)
    EV(Ask, 1)
    EV(Enabled, 2)
END_ENUM()

BEGIN_ENUM(InterruptDecision, uint8_t)
    EV(Use, 0)
    EV(Ignore, 1)
    EV(Ask, 2)
END_ENUM()

BEGIN_ENUM(DynamicModifierType, uint8_t)
    EV(None, 0)
    EV(Consumable, 1)
    EV(ItemSpell, 2)
    EV(Spell, 3)
    EV(ToggledPassive, 4)
END_ENUM()

BEGIN_BITMASK(BoundBaseFlags, uint8_t)
    EV(UseTranslateOverride, 1)
    EV(UseOnDistance, 2)
END_ENUM()

BEGIN_BITMASK(BoundFlags, uint16_t)
    EV(ItemWalkThroughBlock, 0x01)
    EV(CharacterWalkThroughBlock, 0x02)
    EV(ItemSeeThroughBlock, 0x04)
    EV(CharacterSeeThroughBlock, 0x08)
    EV(ItemShootThroughBlock, 0x10)
    EV(CharacterShootThroughBlock, 0x20)
    EV(Interactable, 0x40)
    EV(InvisibleTrap, 0x80)
    EV(Indestructible, 0x100)
    EV(SurfaceBlocker, 0x200)
    EV(SurfaceCloudBlocker, 0x400)
    EV(CanClimbOn, 0x800)
    EV(WalkOn, 0x1000)
    EV(IsPlatformOwner, 0x2000)
END_ENUM()

BEGIN_BITMASK(AiBaseFlags, uint32_t)
    EV(Blocker, 0x01)
    EV(WalkBlock, 0x02)
    EV(WalkBlock2, 0x04)
    EV(ShootBlock, 0x08)
    EV(WalkBlockCharacter, 0x10)
    EV(ShootBlockCharacter, 0x20)
    EV(SubgridEdge, 0x40) // Deprecated name
    EV(Climbable, 0x40)
    EV(WalkBlockItem, 0x80)
    EV(ShootBlockItem, 0x100)
    EV(GroundSurfaceBlock, 0x200)
    EV(CloudSurfaceBlock, 0x400)
    EV(UnknownBlock1, 0x800)
    EV(Slope, 0x800)
    EV(UnknownBlock2, 0x1000) // Deprecated name
    EV(SlopeBlock, 0x1000)
    EV(UnknownBlock3, 0x2000) // Deprecated name
    EV(PaintedBlock, 0x2000)
    EV(UnknownBlock4, 0x4000) // Deprecated name
    EV(StaticBlock, 0x4000)
    EV(UnknownBlock5, 0x8000) // Deprecated name
    EV(GenericBlock, 0x8000)
    EV(Indestructible, 0x10000)
    EV(CanClimbOn, 0x20000)
    EV(Trap, 0x40000)
    EV(Portal, 0x80000)
    EV(PortalTarget, 0x100000)
    EV(Door, 0x400000)
    EV(DoorItemBlock, 0x800000)
END_ENUM()

// Note: Starts at 0x400000000000 in the 64-bit value
BEGIN_BITMASK(AiExtraFlags, uint32_t)
    EV(Obscured, 0x01)
    EV(LitBySunlight, 0x02)
    EV(HalfLit, 0x04)
    EV(Ledge, 0x08)
    EV(LedgeN, 0x10)
    EV(LedgeE, 0x20)
    EV(LedgeS, 0x40)
    EV(LedgeW, 0x80)
    EV(Chasm, 0x100)
    EV(SubgridEdge, 0x200)
    EV(AllowsSunlight, 0x400)
    EV(SubgridIntersection, 0x800)
    EV(Lit, 0x1000)
    EV(LitByAtmosphere, 0x2000)
    EV(PaintedGroundSurface, 0x4000)
    EV(PaintedCloudSurface, 0x8000)
    EV(FixedGroundSurface, 0x10000)
    EV(FixedCloudSurface, 0x20000)
END_ENUM()

BEGIN_BITMASK(PhysicsGroupFlags, uint32_t)
    EV(Group01, 0x01)
    EV(Group02, 0x02)
    EV(Group04, 0x04)
    EV(Group08, 0x08)
    EV(Terrain, 0x10)
    EV(CanWalkOn, 0x20)
    EV(Group40, 0x40)
    EV(Group80, 0x80)
    EV(Group100, 0x100)
    EV(Character, 0x200)
    EV(Item, 0x400)
    EV(Scenery, 0x800)
    EV(Fadeable, 0x1000)
    EV(Wadable, 0x2000)
    EV(Dead, 0x4000)
    EV(VisibleItem, 0x8000)
    EV(CanClickThrough, 0x10000)
    EV(CanSeeThrough, 0x20000)
    EV(PointerBlocker, 0x40000)
    EV(Dragging, 0x80000)
    EV(CanClimbOn, 0x100000)
    EV(IsDecorative, 0x200000)
    EV(CanShootThrough, 0x400000)
    EV(CanSeeThrough2, 0x800000)
    EV(ClimbableTileConstruction, 0x1000000) // Legacy name
    EV(CanClimbOnStatic, 0x1000000)
    EV(DisableBoundCollision, 0x2000000)
    EV(PlatformOwner, 0x4000000)
    EV(Falling, 0x8000000)
    EV(Group10000000, 0x10000000)
    EV(TimelinePreview, 0x20000000)
    EV(CanShineThrough, 0x40000000)
    EV(AttackableWhenClickThrough, 0x80000000)
END_ENUM()

BEGIN_BITMASK(PhysicsExtraFlags, uint32_t)
    EV(ShootThroughGargantuan, 0x01)
    EV(ShootThroughHuge, 0x02)
    EV(ShootThroughLarge, 0x04)
    EV(ShootThroughMedium, 0x08)
    EV(ShootThroughSmall, 0x10)
    EV(ShootThroughTiny, 0x20)
    EV(BlockAoEDamage, 0x40)
END_ENUM()

BEGIN_BITMASK(StatsDirtyFlags, uint64_t)
    EV(AC, 0x01)
    EV(Abilities, 0x02)
    EV(Skills, 0x04)
    EV(Resistances, 0x08)
    EV(ProficiencyBonus, 0x10)
    EV(SavingThrow, 0x20)
    EV(MaxHP, 0x40)
    EV(StatusImmunities, 0x80)
    EV(FootstepsType, 0x200)
    EV(Weight, 0x400)
    EV(TempHP, 0x800)
    EV(Initiative, 0x1000)
    EV(WeaponActionDC, 0x2000)
    EV(MaxPreparedSpells, 0x4000)
    EV(Invulnerability, 0x8000)
    EV(ItemUsability, 0x40000)
    EV(Encumbrance, 0x80000)
    EV(UnarmedAndNaturalArmorType, 0x100000)
    EV(EquipmentSlotAvailability, 0x200000)
    EV(WeaponProperties, 0x400000)
    EV(WeaponAttackRollAbility, 0x800000)
    EV(PhysicalForceRangeBonus, 0x1000000)
    EV(SpellDifficultyCheck, 0x2000000)
    EV(DamageOnWeapon, 0x4000000)
    EV(UseBoostRequests, 0x8000000)
    EV(CurrentHP, 0x10000000)
    EV(AttackRollBonusOnWeapon, 0x20000000)
END_ENUM()

BEGIN_BITMASK(PhysicsType, uint32_t)
    EV(Static, 0x01)
    EV(Dynamic, 0x04)
END_ENUM()

BEGIN_BITMASK(PhysicsHitType, uint32_t)
    EV(Normal, 1)
    EV(DoubleSided, 8)
END_ENUM()

BEGIN_ENUM(CombatPauseSourceType, uint8_t)
    EV(Unknown, 0)
    EV(Script, 1)
END_ENUM()

BEGIN_ENUM(InterruptRequestSource, uint8_t)
    EV(Boost, 0)
    EV(Debug, 1)
    EV(LearnedSpell, 2)
END_ENUM()

BEGIN_ENUM(NarrativeCombatRequestType, uint32_t)
    EV(Create, 0)
    EV(Destroy, 1)
    EV(SetInNarrativeCombat, 2)
    EV(ClearInNarrativeCombat, 3)
END_ENUM()

BEGIN_ENUM(PassiveSourceType, uint8_t)
    EV(Feat, 0)
    EV(Progression, 1)
    EV(Progression2, 2)
    EV(Status, 3)
    EV(Equipment, 4)
    EV(Stats, 5)
    EV(Tadpole, 6)
    EV(Script, 7)
    EV(Debug, 8)
END_ENUM()

BEGIN_ENUM(WeaponSetType, uint8_t)
    EV(Melee, 0)
    EV(Ranged, 1)
    EV(Unknown, 2)
END_ENUM()

BEGIN_BITMASK(SpellCastOptions, uint32_t)
    EV(IgnoreHasSpell, 0x1)
    EV(IgnoreCastChecks, 0x2)
    EV(IgnoreSpellRolls, 0x4)
    EV(IsReaction, 0x8)
    EV(NoMovement, 0x10)
    EV(AvoidAoO, 0x20)
    EV(DestroySource, 0x40)
    EV(Immediate, 0x80)
    EV(Silent, 0x100)
    EV(IgnoreTargetChecks, 0x200)
    EV(IsPreview, 0x400)
    EV(IsHoverPreview, 0x800)
    EV(ShowPrepareAnimation, 0x1000)
    EV(Forced, 0x2000)
    EV(IsRoll, 0x4000)
    EV(IsInterrupt, 0x8000)
    EV(FromClient, 0x10000)
    EV(NoUnsheath, 0x20000)
    EV(CheckProjectileTargets, 0x40000)
    EV(AvoidDangerousAuras, 0x80000)
    EV(Unknown100000, 0x100000)
END_ENUM()

BEGIN_ENUM(SpellCastFailReason, uint8_t)
    EV(None, 0)
    EV(Canceled, 2)
    EV(InvalidSpell, 3)
    EV(MissingTarget, 4)
    EV(CantCast, 5)
    EV(InvalidTarget, 6)
    EV(CantUseSource, 7)
    EV(CantSpendUseCosts, 8)
    EV(MissingAnimations, 9)
    EV(MissingTargetPosition, 10)
    EV(TooManyTargets, 11)
    EV(TooFewTargets, 12)
    EV(MissingCaster, 13)
    EV(DeadTarget, 14)
    EV(EqualTargets, 15)
    EV(CantFindCastPosition, 16)
    EV(BlockedRequiredMove, 17)
    EV(InvalidCastPosition, 18)
    EV(CantMove, 19)
    EV(MovementFailed, 20)
    EV(ServerTimeout, 21)
    EV(DeadCaster, 22)
    EV(Countered, 23)
    EV(SpellRollFailure, 24)
    EV(UnsheathFailure, 25)
END_ENUM()

BEGIN_ENUM(RestCancelReason, uint8_t)
    EV(None, 0)
    EV(Validation, 1)
    EV(Script, 2)
    EV(RestTypeSelectionCanceled, 3)
    EV(ValidationChangedDuringSelection, 4)
    EV(AnotherPlayerCanceled, 5)
    EV(ForcedCancelOnLoad, 6)
END_ENUM()

BEGIN_ENUM(LongRestState, uint8_t)
    EV(Created, 0)
    EV(Validation, 1)
    EV(RestTypeRequest, 2)
    EV(ScriptPhaseStart, 3)
    EV(ScriptPhaseRequested, 4)
    EV(ScriptPhasePending, 5)
    EV(ScriptPhaseTimeout, 6)
    EV(ScriptPhaseDenied, 7)
    EV(Cancelled, 8)
    EV(Finished, 9)
    EV(ReadyForDelete, 10)
END_ENUM()

BEGIN_ENUM(LongRestUserState, uint8_t)
    EV(Initial, 0)
    EV(StartingRest, 1)
    EV(RequestingData, 2)
    EV(Resting, 3)
    EV(WaitingConfirmation, 4)
    EV(WaitingOthers, 5)
    EV(Finishing, 6)
    EV(Finished, 7)
END_ENUM()

BEGIN_ENUM(LongRestType, uint8_t)
    EV(None, 0)
    EV(Full, 1)
    EV(Exhausted, 2)
    EV(CanceledByOwner, 3)
    EV(CanceledByObserver, 4)
    EV(CantRest, 5)
END_ENUM()

BEGIN_ENUM(SpellAnimationCastEvent, uint8_t)
    EV(Prepare, 0)
    EV(Cast, 1)
    EV(CastContinue, 2)
    EV(Cancel, 3)
    EV(Finished, 4)
    EV(TargetingInfo, 5)
END_ENUM()

BEGIN_ENUM(ArmorSetState, uint8_t)
    EV(Normal, 0)
    EV(Vanity, 1)
END_ENUM()

BEGIN_ENUM(IsTradableType, uint8_t)
    EV(Default, 0)
    EV(Tradable, 1)
    EV(NonTradable, 2)
END_ENUM()

BEGIN_ENUM(InventoryLockChange, uint8_t)
    EV(Lock, 0)
    EV(Unlock, 1)
    EV(Toggle, 2)
END_ENUM()

BEGIN_ENUM(InventoryType, uint8_t)
    EV(Main, 0)
    EV(Equipment, 1)
    EV(Trade, 2)
    EV(Reward, 3)
    EV(KeyChainTest, 4)
    EV(PotionBeltTest, 5)
    EV(Other, 6)
END_ENUM()

BEGIN_ENUM(UnequipType, uint8_t)
    EV(ToParentInventory, 0)
    EV(ToParentOrCreatedInventory, 1)
    EV(SwapHands, 2)
END_ENUM()

BEGIN_ENUM(ContainerInteractionType, uint8_t)
    EV(Sort, 0)
    EV(Scatter, 1)
END_ENUM()

BEGIN_ENUM(DualWieldingRequestType, uint8_t)
    EV(OnItemEquipped, 1)
    EV(OnItemEquipped2, 2)
    EV(ActionResourceChanged, 3)
    EV(BoostChanged, 4)
    EV(ToggleDualWielding, 5)
    EV(ToggleOnCharacterControlChanged, 6)
END_ENUM()

BEGIN_ENUM(DisplayNameType, uint8_t)
    EV(None, 0)
    EV(Unknown, 1)
    EV(Stats, 2)
    EV(Transform, 4)
    EV(CharacterCreation, 5)
    EV(Template, 6)
    EV(Story, 8)
END_ENUM()

BEGIN_ENUM(DisplayTitleType, uint8_t)
    EV(Transform, 0)
    EV(Story, 1)
    EV(Template, 2)
END_ENUM()

BEGIN_ENUM(GravityType, uint8_t)
    EV(Enabled, 0)
    EV(Disabled, 1)
    EV(DisabledUntilMove, 2)
END_ENUM()

BEGIN_ENUM(ApprovalReactionScope, uint8_t)
    EV(Global, 0)
    EV(Local, 1)
    EV(Dialog, 2)
END_ENUM()

BEGIN_ENUM(IdentityType, uint8_t)
    EV(Male, 0)
    EV(Female, 1)
END_ENUM()

BEGIN_ENUM(BodyType, uint8_t)
    EV(Male, 0)
    EV(Female, 1)
END_ENUM()

BEGIN_ENUM(SpellCastPhase, uint8_t)
    EV(None, 0)
    EV(ValidationStart, 1)
    EV(ValidationFindCastPosition, 2)
    EV(ValidationFindCastEndPosition, 3)
    EV(ValidationPathfind, 4)
    EV(ValidationFinished, 5)
    EV(MovementStart, 6)
    EV(MovementUpdate, 7)
    EV(MovementSteering, 8)
    EV(MovementFinished, 9)
    EV(PrecalculationStart, 10)
    EV(PrecalculationFinished, 11)
    EV(PrepareStart, 12)
    EV(PrepareUpdate, 13)
    EV(PrepareEnd, 14)
    EV(PrepareFinished, 15)
    EV(LogicExecutionStart, 16)
    EV(LogicExecutionUpdate, 17)
    EV(LogicExecutionInterrupted, 18)
    EV(LogicExecutionEnd, 19)
    EV(LogicExecutionFinished, 20)
    EV(Finished, 21)
END_ENUM()


BEGIN_ENUM(AiActionType, uint8_t)
    EV(None, 0)
    EV(Consume, 1)
    EV(Spell, 2)
    EV(Fallback, 4)
    EV(Seek, 5)
    EV(WeaponPickUp, 6)
    EV(UseItem, 7)
END_ENUM()

BEGIN_BITMASK(AiActionFlags, uint32_t)
    EV(MoveCompleted, 0x01)
    EV(CanExecuteThisTurn, 0x02)
    EV(IgnoreMovementScore, 0x04)
    EV(HasPositionScore, 0x08)
    EV(HasPosition, 0x10)
    EV(HasMoveSpells, 0x20)
    EV(CanReachTarget, 0x40)
    EV(PlayFallbackDialogue, 0x80)
    EV(MovementCalculated, 0x100)
    EV(IsFromItem, 0x200)
    EV(RemoveCastPositionScore, 0x400)
    EV(MoveToSecondTarget, 0x800)
    EV(CanCastWithoutMovement, 0x1000)
    EV(FallbackJump, 0x2000)
    EV(TriggeredFallbackJump, 0x4000)
    EV(FallbackSpell, 0x8000)
    EV(FallbackAttackBlocker, 0x10000)
    EV(HasPathfindTrajectory, 0x20000)
    EV(NoSafeReachablePosition, 0x40000)
END_ENUM()


BEGIN_ENUM_NS(esv::spell_cast, SystemEventType, ServerSpellCastSystemEventType, uint8_t)
    EV(Finish, 0x0)
    EV(FinishRemove, 0x1)
    EV(PreparationStart, 0x2)
    EV(PreparationEnd, 0x3)
    EV(LogicExecutionStart, 0x4)
    EV(LogicExecutionStartRemove, 0x5)
    EV(LogicExecutionEnd, 0x6)
    EV(MoveDuringCastInitialize, 0x7)
    EV(MoveDuringCastStart, 0x8)
    EV(MoveDuringCastUpdate, 0x9)
    EV(MoveDuringCastEnd, 0xA)
    EV(TargetHit, 0xB)
    EV(TargetHitRemove, 0xC)
    EV(TargetHitInterruptRemove, 0xD)
    EV(Cast, 0xE)
    EV(CastRemove, 0xF)
    EV(CastHit, 0x10)
    EV(CastHitRemove, 0x11)
    EV(CastTextKey, 0x12)
    EV(CastTextKeyRemove, 0x13)
    EV(CastTextKeyIndexIncrease, 0x14)
    EV(WeaponSetRequest, 0x15)
    EV(IsCastingRemove, 0x16)
    EV(DestroySpellCastEntity, 0x17)
    EV(DestroyEventRemove, 0x18)
    EV(JumpStart, 0x19)
    EV(PlayAnimation, 0x1A)
    EV(MovementEnd, 0x1B)
    EV(TargetReactionRemove, 0x1C)
    EV(TargetsChanged, 0x1D)
    EV(AddProjectileCache, 0x1E)
    EV(AddZoneRange, 0x1F)
    EV(AddSharedMovement, 0x20)
    EV(AddServerMovement, 0x21)
    EV(AddMovementInfo, 0x22)
    EV(PreviewEnd, 0x23)
    EV(CounteredSpellCast, 0x24)
    EV(SpellRollAbort, 0x25)
    EV(PrecalculationEnd, 0x26)
    EV(PrecalculatedConditionals, 0x27)
    EV(ThrowPickupPositionChanged, 0x28)
    EV(PhaseChange, 0x29)
    EV(CacheFlagChange, 0x2A)
    EV(UseCostsChange, 0x2B)
    EV(InitializeRolls, 0x2C)
    EV(AdvanceRoll, 0x2D)
    EV(AdvanceReaction, 0x2E)
    EV(FireProjectile, 0x2F)
    EV(ChangeProjectileDelayTimer, 0x30)
    EV(PopDelayedProjectile, 0x31)
    EV(InitializeIntermediateTargets, 0x32)
    EV(ChangeCastHitDelayTimer, 0x33)
    EV(AddDelayedCastHit, 0x34)
    EV(PopDelayedCastHit, 0x35)
    EV(ChangeCastPosition, 0x36)
    EV(ChangeCastEndPosition, 0x37)
    EV(ChangeMovementTransactionID, 0x38)
    EV(CreateZone, 0x39)
    EV(DoTeleport, 0x3A)
    EV(CreateWall, 0x3B)
    EV(CastOptionsChange, 0x3C)
    EV(TargetsChange, 0x3D)
    EV(PathfindRequest, 0x3E)
    EV(PathfindResults, 0x3F)
    EV(PathfindCleanup, 0x40)
    EV(ChangeStoryActionId, 0x41)
    EV(InterruptStart, 0x42)
    EV(InterruptEventChange, 0x43)
    EV(InterruptApplyResult, 0x44)
    EV(AdvanceInterruptAnimationIndex, 0x45)
    EV(InterruptEnd, 0x46)
    EV(PrecalculatedInterruptors, 0x47)
    EV(PushPausedAnimationEvents, 0x48)
    EV(PurgePausedAnimationEvents, 0x49)
    EV(ZoneRangeUpdate, 0x4A)
    EV(UpdateConditionals, 0x4B)
    EV(UpdateInterruptors, 0x4C)
    EV(LogicExecutionUpdate, 0x4D)
    EV(PreparationStarting, 0x4E)
END_ENUM_NS()


BEGIN_ENUM_NS(esv::concentration, ConcentrationRequestType, ServerConcentrationRequestType, uint8_t)
    EV(RemoveTarget, 0x1)
    EV(AddTarget, 0x2)
END_ENUM_NS()


BEGIN_ENUM(PhotoModeAnimationState, uint8_t)
    EV(None, 0)
    EV(StaticPose, 1)
    EV(AnimationPlaying, 2)
    EV(AnimationPaused, 3)
END_ENUM()
