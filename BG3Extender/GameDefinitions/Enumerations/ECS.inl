...
BEGIN_BITMASK(InventoryLockChange, uint8_t)
    EV(Lock, 0)
    EV(Unlock, 1)
    EV(Toggle, 2)
END_ENUM()

BEGIN_BITMASK(InventoryType, uint8_t)
    EV(Main, 0)
    EV(Equipment, 1)
    EV(Trade, 2)
    EV(Reward, 3)
...
BEGIN_BITMASK(UnequipType, uint8_t)
    EV(ToParentInventory, 0)
    EV(ToParentOrCreatedInventory, 1)
    EV(SwapHands, 2)
END_ENUM()

BEGIN_BITMASK(ContainerInteractionType, uint8_t)
    EV(Sort, 0)
    EV(Scatter, 1)
END_ENUM()
