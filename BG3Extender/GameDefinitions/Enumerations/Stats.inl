...
BEGIN_ENUM(RollModeType, uint8_t)
    EV(Random, 0)
    EV(WeightedRandom, 1)
    EV(Horrible, 2)
    EV(Amazing, 3)
...
BEGIN_ENUM(RollStreamType, uint8_t)
    EV(Generic, 0)
    EV(Dialog, 1)
    EV(PlayerSpell, 2)
    EV(NPCSpell, 3)
...
BEGIN_BITMASK(CriticalHitAttackFlags, uint8_t)
    EV(AttackTarget, 1)
    EV(AttackRoll, 2)
END_ENUM()

BEGIN_BITMASK(CriticalHitSuccessFlags, uint8_t)
    EV(Success, 1)
    EV(SuccessNever, 2)
    EV(Failure, 4)
    EV(FailureNever, 8)
...
BEGIN_BITMASK(HealingType, uint8_t)
    EV(Living, 1)
    EV(Guaranteed, 2)
    EV(Construct, 4)
    EV(Undead, 8)
...
BEGIN_ENUM(ProgressionSelectorType, uint8_t)
    EV(None, 0)
    EV(Race, 1)
    EV(SubRace, 2)
    EV(Class, 3)
