...
template <class T>
constexpr typename std::enable_if_t<IsBitfieldV<T>, T> operator | (T lhs, T rhs)
{
...
template <class T>
constexpr typename std::enable_if_t<IsBitfieldV<T>, T> operator ~ (T v)
{
	return static_cast<T>(~static_cast<std::underlying_type_t<T>>(v));
}

template <class T>
constexpr typename std::enable_if_t<IsBitfieldV<T>, bool> operator ! (T v)
{
