...
public:
    ...
    {
        if constexpr (std::is_base_of_v<OpaqueNewIndexable, T>) {
            StackCheck _(L, 0);
            auto self = lua_get_opaque_cppvalue(L, 1, T::MetaTag);
            return T::NewIndex(L, self);
        } else if constexpr (std::is_base_of_v<NewIndexable, T>) {
            StackCheck _(L, 0);
            auto self = lua_get_cppvalue(L, 1, T::MetaTag);
            return T::NewIndex(L, self);
        } else {
    ...
    {
        if constexpr (std::is_base_of_v<OpaqueIterable, T>) {
            auto self = lua_get_opaque_cppvalue(L, 1, T::MetaTag);
            return T::OpaquePairs(L, self);
        } else if constexpr (std::is_base_of_v<Iterable, T>) {
            auto self = lua_get_cppvalue(L, 1, T::MetaTag);
    ...
    static int NextProxy(lua_State* L)
    {
        if constexpr (!std::is_base_of_v<Iterable, T>) {
            return luaL_error(L, "Not iterable!");
        } else if constexpr (std::is_base_of_v<OpaqueIterable, T>) {
