...
#define BY_VAL_ARRAY_HELPERS(ty) char const* const ArrayProxyHelpers<ty>::TypeName = #ty;
	...
	int ArrayProxy::Index(lua_State* L)
	{
		...
		if (!lifetime_.IsAlive()) {
			luaL_error(L, "Attempted to read dead array of type '%s'", impl->GetTypeName());
			push(L, nullptr);
			return 1;
		}
	...
	int ArrayProxy::NewIndex(lua_State* L)
	{
		...
		auto impl = GetImpl();
		if (!lifetime_.IsAlive()) {
			luaL_error(L, "Attempted to write dead array of type '%s'", impl->GetTypeName());
			return 0;
		}
	...
	int ArrayProxy::Next(lua_State* L)
	{
		auto impl = GetImpl();
		if (!lifetime_.IsAlive()) {
			luaL_error(L, "Attempted to iterate dead array of type '%s'", impl->GetTypeName());
			return 0;
		}
