...
private:
    ...
    void RunHandler(ServerState& lua, uint64_t nodeRef, RegistryEntry func, TuplePtrLL* tuple);
    void RunHandlers(uint64_t nodeRef, OsiArgumentDesc* tuple);
    void RunHandler(ServerState& lua, uint64_t nodeRef, RegistryEntry func, OsiArgumentDesc* tuple);
};
